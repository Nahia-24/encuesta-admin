    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0">
                <form method="GET" action="{{ route('city.index') }}" class="relative w-56 text-slate-500">
            ...
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px]">
                <x-base.table.thead>
                    <x-base.table.tr>
                        ...
                        <x-base.table.th class="whitespace-nowrap border-b-0 text-center">Ciudad</x-base.table.th>
                        <x-base.table.th class="whitespace-nowrap border-b-0 text-center">Provincia</x-base.table.th>
                ...
                <x-base.table.tbody>
                    @foreach ($cities as $city)
                        <x-base.table.tr class="intro-x bg-white dark:bg-darkmode-600 rounded-[0.6rem] shadow-sm">
                            <x-base.table.td class="border-y-0 first:rounded-l-[0.6rem] align-middle">{{ $city->id }}</x-base.table.td>
                            <x-base.table.td class="text-center border-y-0 align-middle">{{ $city->code_dane }}</x-base.table.td>
                            <x-base.table.td class="text-center border-y-0 align-middle">{{ $city->name }}</x-base.table.td>
                            <x-base.table.td class="text-center border-y-0 align-middle">{{ $city->provincia }}</x-base.table.td>
                            <x-base.table.td class="text-center border-y-0 align-middle">
                                <div class="flex justify-center gap-3">
                                    <a href="{{ route('city.edit', ['id' => $city->id]) }}" class="flex items-center text-primary font-medium hover:underline">
                                        <x-base.lucide class="w-4 h-4 mr-1" icon="Edit" /> Editar
                                    </a>
                                    <a href="{{ route('city.delete', ['id' => $city->id]) }}" class="flex items-center text-danger font-medium hover:underline" data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal">
