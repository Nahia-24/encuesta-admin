...
@section('subcontent')
    ...
    <div class="intro-y box mt-5">
        <div class="flex flex-col sm:flex-row justify-between gap-6 m-4">
            ...
            <div class="sm:w-1/3 flex flex-col justify-end">
                <label class="invisible block text-sm font-medium text-gray-700">Botón</label>
                <x-base.button id="generateCouponsButton" class="w-full shadow-md mt-1" variant="primary">
            ...
            <x-base.table.tbody id="couponsTableBody">
                @foreach ($coupons as $key => $coupon)
                    <x-base.table.tr class="intro-x">
                        <x-base.table.td
                            class="box w-40 rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">{{ ($coupons->currentPage() - 1) * $coupons->perPage() + $key + 1 }}</x-base.table.td>
                        <x-base.table.td
                            class="box rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">{{ $coupon->numeric_code }}</x-base.table.td>
                        <x-base.table.td
                            class="box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">{{ $coupon->ticketType->name }}</x-base.table.td>
                        <x-base.table.td
                            class="box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600 {{ $coupon->is_consumed ? 'bg-red-500 text-white' : 'bg-green-500 text-white' }}">
                            {{ $coupon->is_consumed ? 'No Disponible' : 'Disponible' }}
                        </x-base.table.td>
                        <x-base.table.td
                            class="box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">{{ $coupon->created_at->format('d/m/Y') }}</x-base.table.td>
                        <x-base.table.td
                            ...
                            <div class="flex items-center justify-center">
                                <x-base.tippy content="Generar PDF" class="mr-1">
                                    <a class="text-info" href="{{ route('coupon.pdf', ['id' => $coupon->id]) }}"
                                        target="_blank">
                                        <x-base.lucide icon="FileText" />
                                    </a>
                                </x-base.tippy>
                                @if (!$coupon->is_consumed)
                                    <!-- Solo mostrar el botón si el cupón no ha sido consumido -->
                                    <x-base.tippy content="Eliminar Cupón" class="ml-1">
                                        <form action="{{ route('coupon.delete', ['id' => $coupon->id]) }}" method="POST"
                                            onsubmit="return confirm('¿Estás seguro de que deseas eliminar este cupón?');">
                                            @csrf
                                            @method('DELETE')
