                    ...
                    @foreach ($eventos as $evento)
                        <x-base.table.tr class="intro-x">
                                ...
                                <div class="flex items-center justify-center">
                                    ...
                                    <x-base.tippy content="Editar" class="mr-1">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('event.edit', ['id' => $evento->id]) }}">
                                            <x-base.lucide class="mx-auto block" icon="edit" />
                                        </a>
                                    </x-base.tippy>

                                    @if (
                                        $evento->registration_parameters == 'null' ||
                                            $evento->registration_parameters == null ||
                                            $evento->registration_parameters == '[]')
                                        <x-base.tippy content="Registro Parametros (Sin Parametros)" class="mr-1">
                                            <a class="mr-3 flex items-center"
                                                href="{{ route('events.setRegistrationParameters', $evento->id) }}">
                                                <x-base.alert class="flex items-center" variant="soft-pending">
                                                    <x-base.lucide class="mx-auto block" icon="Book" />
                                                </x-base.alert>
                                            </a>
                                        </x-base.tippy>
                                    @else
                                    ...
                                    <x-base.tippy content="Generar codigos cortesía" class="mr-1">
                                        <a class="mr-3 flex items-center" href="{{ route('coupons.index', $evento->id) }}">
                                            <x-base.lucide class="mx-auto block" icon="Tag" />
                                        </a>
                                    </x-base.tippy>
                                    @if ($evento->public_link)
                                        <x-base.tippy content="Ver enlace" class="mr-2">
                                            <a class="mr-3" href="{{ route('event.register', $evento->public_link) }}"
                                                target="_blank">
                                                <x-base.lucide class="mx-auto block" icon="ExternalLink" />
                                            </a>
                                        </x-base.tippy>
                                    @else
                                        <x-base.tippy content="Generar Enlace Público" class="mr-2">
                                            <form action="{{ route('event.generatePublicLink', $evento->id) }}"
                                                method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-primary ">
                                                    <x-base.lucide class="mx-auto block" icon="Link" />
                                                </button>
                                            </form>
                                        </x-base.tippy>
                                    @endif
                                    ...
                                    <x-base.tippy content="Lista Asistentes" class="mr-1">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('eventAssistant.index', ['idEvent' => $evento->id]) }}">
                                            <x-base.lucide class="mx-auto block" icon="Users" />
                                        </a>
                                    </x-base.tippy>

                                    <x-base.tippy content="Recaudos" class="mr-1">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('payments.index', ['idEvent' => $evento->id]) }}">
                                            <x-base.lucide class="mx-auto block" icon="DollarSign" />
                                        </a>
                                    </x-base.tippy>
                                        ...
                                        @else
                                            ...
                                            <div class="image-fit zoom-in h-10 w-10">
                                                <x-base.tippy
                                                    ...
                                                    as="img" content="No image available" />
                                            </div>
                                        @endif
                                    </div>
                            </x-base.table.td>
                            ...
                            <x-base.table.td
                                ...
                                onclick="showDescriptionModal('{{ $evento->description }}')" style="cursor: pointer;">
                            ...
                            <x-base.table.td
                                class="box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                                @if ($evento->city && $evento->city->department)
